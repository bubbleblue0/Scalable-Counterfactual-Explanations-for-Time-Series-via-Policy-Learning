...
def sparsity_loss(X_hat_cf: tf.Tensor, X: tf.Tensor) -> Dict[str, tf.Tensor]:
    ...
    -------
        L1 sparsity loss.
    """
    beta = 0.5 # adjust trade-off between L1 and L2
    L1_loss = tf.reduce_mean(tf.abs(X_hat_cf - X))  # L1
    L2_loss = tf.reduce_mean(tf.square(X_hat_cf - X))  # L2
    sparsity_loss = beta * L1_loss + L2_loss # L1 + L2 elastic net loss
...
def update_actor_critic(encoder: keras.Model,
    ...
    # Ensure reward dtype is float32
    R_tilde = tf.cast(R_tilde, dtype=tf.float32)
    ...
    with tf.GradientTape() as tape_actor:
        ...
        # Compute actors' loss. policy gradient loss (negative critic value)
