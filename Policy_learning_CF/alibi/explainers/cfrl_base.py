...
DEFAULT_BASE_PARAMS = {
    ...
    "use_replay": True,  # flag enabling experience replay. If False, updates use most recent batch directly.
}
"""
Default Counterfactual with Reinforcement Learning parameters.
    ...
    the replay buffer is full enough for useful updates. Ignored when ``'use_replay'`` is False.
    ...
    - ``'use_replay'``: bool, whether to enable experience replay. If True, batches are stored in a replay buffer and \
    updates sample past experience after ``'update_after'`` warm-up. If False, every batch can trigger immediate \
    updates (subject to ``'update_every'``) using only the most recent batch; ``'update_after'`` is ignored.
"""
...
_PARAM_TYPES = {
    "primitives": [
        ...
        "critic_hidden_dim", "use_replay",  # <-- add
    ],
    "complex": [
...
class CounterfactualRL(Explainer, FitMixin):
    ...
    def fit(self, X: np.ndarray) -> "Explainer":
        ...
        use_replay: bool = bool(self.params.get("use_replay", True))
        if not use_replay and self.params.get("update_after", 0) > 0:
            logger.warning("'update_after' is ignored because 'use_replay' is False.")

        # Define replay buffer only if enabled (this will deal only with numpy arrays).
        replay_buff = ReplayBuffer(size=self.params["replay_buffer_size"]) if use_replay else None
        ...
        # After autoencoder training, check embedding space coverage
        X_preprocessed = self.params["encoder_preprocessor"](X)
        Z_all = self.backend.encode(X_preprocessed, **self.params)
        Y_all = self.params["predictor"](X)

        # Separate embeddings by class
        class_0_mask = np.argmax(Y_all, axis=1) == 0
        class_1_mask = np.argmax(Y_all, axis=1) == 1

        Z_class_0 = Z_all[class_0_mask]
        Z_class_1 = Z_all[class_1_mask]

        print(f"Class 0 samples: {np.sum(class_0_mask)}")
        print(f"Class 1 samples: {np.sum(class_1_mask)}")
        print(f"Class 0 embedding mean: {np.mean(Z_class_0, axis=0)}")
        print(f"Class 1 embedding mean: {np.mean(Z_class_1, axis=0)}")
        print(f"Embedding space separation: {np.linalg.norm(np.mean(Z_class_0, axis=0) - np.mean(Z_class_1, axis=0))}")
        ...
        for step in tqdm(range(self.params["train_steps"])):
            ...
            # Ensure reward dtype is float32 for TensorFlow backend compatibility
            R_tilde = np.asarray(R_tilde, dtype=np.float32)
            data.update({"R_tilde": R_tilde, "Y_m_cf_tilde": Y_m_cf_tilde})

            # Store experience in the replay buffer or prepare current batch sample.
            data = {key: self.backend.to_numpy(data[key]) for key in data.keys()}
            if use_replay:
                replay_buff.append(**data)

            # Determine if we can perform updates this step.
            can_update = (step % self.params['update_every'] == 0) and \
                         (step > self.params["update_after"] if use_replay else True)
            if can_update:
                for i in range(self.params['update_every']):
                    # Sample batch of experience form the replay buffer or use current data.
                    sample = replay_buff.sample() if use_replay else {key: data[key] for key in [
                        "X", "Y_m", "Y_t", "Z", "Z_cf_tilde", "C", "R_tilde"]}

                    if not use_replay:
                        # Guarantee correct dtype when bypassing replay buffer
                        sample["R_tilde"] = np.asarray(sample["R_tilde"], dtype=np.float32)

                    if "C" not in sample:
                        sample["C"] = None

                    # Initialize actor/critic if required (tensorflow only) using current sample tensors.
                    ...
                    # Decode fresh counterfactual for consistency loss with current actor.
